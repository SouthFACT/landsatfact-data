...
cur.execute(
    "SELECT metadata.scene_id, metadata.browse_url "+
        ...
        "WHERE substring(metadata.scene_id, 4, 6) = wrs.wrs2_code AND now()::date - acquisition_date::date <= 30 AND now()::date - acquisition_date::date  > 0 AND browse_url IS NOT null;"
)
